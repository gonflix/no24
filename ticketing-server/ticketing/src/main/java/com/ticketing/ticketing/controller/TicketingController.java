    ...
    public ResponseEntity<ApiResponse> requestPayment(@Valid @RequestBody PaymentRequest request,
        ...
        if (reservation.isEmpty()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.fail("유효하지 않은 예약입니다."));
        }
        if (!reservation.get().userId().equals(userId)) {
            log.warn("requestPayment: userId mismatch. expected={}, actual={}", reservation.get().userId(), userId);
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(ApiResponse.fail("접근 권한이 없습니다."));
        }

        try {
            paymentProducer.send(new PaymentRequestedEvent(
                    request.reservationId(), userId, request.amount(), request.paymentMethod()));
        } catch (IllegalStateException e) {
            log.error("requestPayment: failed to publish event. reservationId={}", request.reservationId(), e);
